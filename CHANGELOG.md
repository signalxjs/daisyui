...
- `ThemeConfigurator` now renders its custom slot layout for **any** non-empty default-slot content. Core 0.15 widened declared slot returns to `JSXElement | JSXElement[] | null`, and the previous check (`slotContent.length > 0`) only recognized arrays — a slot fill arriving as a single node, string or number (including `0`) now also selects the custom layout instead of falling through to the built-in configurator UI. Fills JSX renders as nothing — empty arrays, empty strings, booleans and `null`/`undefined` — still fall through as before.
...
- **BREAKING (packaging):** Retargeted the SignalX core peer range to the `0.13.x` line: the `sigx`, `@sigx/reactivity`, `@sigx/runtime-core` and `@sigx/runtime-dom` peers move from `>=0.12.0 <0.13.0` to `>=0.13.0 <0.14.0` (dev/build deps — including `@sigx/vite` — bumped to `^0.13.0` to match). Consumers must now be on the SignalX core `0.13.x` line — `sigx` and all `@sigx/*` core packages — across their dependency tree. Package managers that enforce peer ranges (e.g. pnpm with its default `strict-peer-dependencies`) refuse to install an out-of-range core; others surface it as a peer-dependency warning.

  `@sigx/daisyui`'s own API is unchanged and it needs no code changes to run on core 0.13: the 0.12→0.13 core changes are additive for a DOM component library. The single-minor pin lives in the `catalog:` block of `pnpm-workspace.yaml`, which `pnpm pack` rewrites to the concrete `^0.13.0` range in the published manifest. See the [SignalX changelog](https://github.com/signalxjs/core/blob/main/CHANGELOG.md) for the full history.
